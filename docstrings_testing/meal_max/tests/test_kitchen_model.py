...
def test_create_song_invalid_price():
    ...
    with pytest.raises(ValueError, match="Invalid meal price: @ \(must be a positive float\)."):
        create_meal(meal="Pasta", cuisine="Italian", price= '@', difficulty="LOW")
...
def test_create_song_invalid_difficulty():
    ...
    with pytest.raises(ValueError, match="Invalid year provided: Easy \(must be a LOW MED HIGH\)."):
        create_meal(meal="Pasta", cuisine="Italian", price= '@', difficulty="Easy")
