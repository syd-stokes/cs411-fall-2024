...
def get_random() -> float:
    """
    Fetches a random decimal number between 0 and 1 from random.org that will be 
    used to determine the winner of the "battle" between two Meal combatants.

    Returns:
        float: The random decimal number fetched from random.org.

    Raises:
        RuntimeError: If the request to random.org fails, times out, or returns an invalid response.
